...
export function AppProvider({ children }) {
  ...
  const loadEmails = useCallback(async (mailboxId, page = 0) => {
    ...
    try {
      const res = await invoke('read_emails', {
  ...
  const selectEmail = useCallback(async (email) => {
    ...
    if (!email.is_read) {
      ...
      try {
        await invoke('mark_read', { email_ids: [email.email_id], read: true, account_id: email.account_id, mailbox_id: email.mailbox_id });
      } catch (_) { /* silent — local state already updated */ }
    }
  }, []);

  const flagEmail = useCallback(async (emailId) => {
    const email = state.emails.find(item => item.email_id === emailId) || state.activeEmail;
  ...
  const bulkMarkRead = useCallback(async (read) => {
    ...
    try {
      await invoke('mark_read', { email_ids: emailIds, read, account_id: state.activeAccountId, mailbox_id: state.activeMailboxId });
  ...
  const search = useCallback((query) => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        const res = await invoke('search_emails', { query, pagination: { offset: 0, limit: 50 } });
